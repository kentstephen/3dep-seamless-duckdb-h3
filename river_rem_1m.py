...
def _(mo):
    mo.md(r"""
    # River REM → H3 Hexagons (1m DEM)

    Relative Elevation Model pipeline using **1m DEM** from USGS 3DEP via
    `seamless-3dep`'s `get_map()` → NHDPlus flowlines via `pynhd` →
    IDW-interpolate river surface → subtract to get height-above-river →
    aggregate to H3 hexagons via DuckDB → render with lonboard.

    Uses `get_map("DEM", ..., res=1)` which hits the USGS WMS export service
    for 1m data where lidar coverage exists. Returns EPSG:3857.

    **Run with:** `uv run marimo edit river_rem_1m.py --sandbox`
    """)
    return
...
def _(
    ...
    def _reproject_bbox(bbox, src_crs, dst_crs):
        """Reproject a (west, south, east, north) bbox between CRSs."""
    ...
    def load_dem(bbox, save_dir, res=1):
        ...
        get_map returns EPSG:3857. We reproject the bbox to 3857 for clipping,
        then return the DEM in 3857 (the rest of the pipeline handles CRS).
        """
    ...
    def aggregate_rem_to_h3(rem, h3_res):
        ...
        Reprojects to 4326 for H3 cell assignment if needed.
        """
        crs = str(rem.rio.crs)
...
def _(mo):
    ...
    Carson River, NV — small bbox for testing 1m DEM retrieval.
    1m coverage depends on USGS lidar projects — not all areas have it.
...
@app.cell
def _(h3):
    # Carson River, NV (small bbox — 1m generates large arrays)
